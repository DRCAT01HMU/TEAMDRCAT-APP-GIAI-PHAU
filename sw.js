const CACHE_NAME = 'tdc-anatomy-v63-v3';
...
self.addEventListener('fetch', event => {
  ...
  const isLibrary = url.host.includes('cdn') || url.host.includes('cdnjs') || url.host.includes('unpkg') || url.pathname.endsWith('.png');

  if (isLibrary) {
    // Cache-First (with background revalidation) for static libraries/icons
    event.respondWith(
    ...
    // Network-First (with Cache Fallback) for HTML pages & manifest to ensure instant updates
    event.respondWith(
      fetch(event.request)
